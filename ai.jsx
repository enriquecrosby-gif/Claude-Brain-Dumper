...
Tags should be lowercase single words or short phrases (e.g. "money", "creative block", "family", "morning fog", "song idea", "product idea"). Maximum 4 tags. Pick what is actually present.
...
// Workshop: turn a dump into a specific output format.
async function aiTransform(text, format, parking = []) {
  const formatPrompts = {
    ...
    article: `Shape this dump into a draft Substack-style article. Length: 600-900 words. Keep their voice — first person, honest, unhurried. Add an evocative title. Find the spine of the piece (the one true thread) and follow it. Do not invent facts or examples; if the dump has a story, use it; if not, write more reflectively. Return:
...
Be kind. No more than 7 items total. Group emotional items as one ("rest" or "feel") rather than listing them.`,
  };
  ...
  const parkingNote = parking.length > 0
    ? `\n\nFor context, here are recent parked ideas (do not force them in, only use if relevant):\n${parking.slice(0, 8).map(p => '- ' + p.text).join('\n')}`
    : '';
...
{
  "themes": [
    {"name": "short theme name (1-3 words, lowercase)", "count": <approx number of entries it appears in>, "note": "one sentence on what this theme really is for them"}
  ],
...
// Encouragement on save — short and real.
async function aiEncouragement(text) {
  const prompt = `${TONE_COMPANION}

The person just saved a brain dump. Give them ONE short sentence of encouragement (under 14 words). No flattery, no clichés, no exclamation marks. Plain and true. Speak directly to them.
